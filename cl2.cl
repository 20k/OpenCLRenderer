...
///SWIZZLE ME BITCH
bool generate_hard_occlusion(float2 spos, float3 lpos, float3 normal, float3 position_to_light, __global uint* light_depth_buffer, int which_cubeface, float3 back_rotated, int shnum)
{
    const float3 r_struct[6] =
    {
        {
...
///__global uint sacrifice_children_to_argument_god
{
    ...
    for(int i=0; i<num_lights; i++)
    {
        ...
        if(l.shadow == 1 && ((which_cubeface = ret_cubeface(global_position, lpos))!=-1)) ///do shadow bits and bobs
        {
            ...
            occluded = generate_hard_occlusion((float2){x, y}, lpos, normal, l2c, light_depth_buffer, which_cubeface, global_position, shnum); ///copy occlusion into local memory?
